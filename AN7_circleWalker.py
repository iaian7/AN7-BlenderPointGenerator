bl_info = {
	"name": "AN7 Circle Walker",
	"author": "Iaian7 - John Einselen",
	"version": (0, 1),
	"blender": (2, 80, 0),
	"location": "Scene (edit mode) > AN7 Tools > Circle Walker",
	"description": "Creates an array of points with radius data",
...
class AN7_Circle_Walker(bpy.types.Operator):
	bl_idname = "an7circlewalker.offset"
	...
	def execute(self, context):
		# Recursion settings
		elements = bpy.context.scene.an7_circle_walker_settings.max_elements # target number of points
		failures = bpy.context.scene.an7_circle_walker_settings.max_failures # maximum number of consecutive failures
		attempts = bpy.context.scene.an7_circle_walker_settings.max_attempts # maximum number of iterations to try and meet the target number of points
		# Properties settings
		dimensions = True if bpy.context.scene.an7_circle_walker_settings.walk_dimensions == "3D" else False
		# shapeX = bpy.context.scene.an7_circle_walker_settings.walk_vector[0] * 0.5 # X distribution radius
		# shapeY = bpy.context.scene.an7_circle_walker_settings.walk_vector[1] * 0.5 # Y distribution radius
		# shapeZ = bpy.context.scene.an7_circle_walker_settings.walk_vector[2] * 0.5 # Z distribution radius
		rMinimum = bpy.context.scene.an7_circle_walker_settings.radius_min # minimum radius of the generated point
		rMaximum = bpy.context.scene.an7_circle_walker_settings.radius_max # maximum radius of the generated point
		rDecay = bpy.context.scene.an7_circle_walker_settings.radius_decay # maximum radius of the generated point
		...
		context.scene.an7_circle_walker_settings.feedback_elements = str(len(points))
		context.scene.an7_circle_walker_settings.feedback_failures = str(failmax)
		context.scene.an7_circle_walker_settings.feedback_attempts = str(iteration)
		context.scene.an7_circle_walker_settings.feedback_time = str(round(time.time() - float(timer), 2))
...
class AN7CircleWalkerPreferences(bpy.types.AddonPreferences):
	bl_idname = __name__
...
class an7CircleWalkerSettings(bpy.types.PropertyGroup):
...
class AN7TOOLS_PT_circle_walker(bpy.types.Panel):
	bl_space_type = "VIEW_3D"
	bl_region_type = "UI"
	bl_category = 'AN7 Tools'
	bl_order = 0
	bl_label = "Circle Walker"
	bl_idname = "AN7TOOLS_PT_circle_walker"
	...
	def draw_header(self, context):
		try:
			...
			print(str(exc) + " | Error in the AN7 Circle Walker panel header")
	...
	def draw(self, context):
		try:
			...
			layout.prop(context.scene.an7_circle_walker_settings, 'walk_dimensions')
			# col=layout.column()
			# col.prop(context.scene.an7_circle_walker_settings, 'walk_vector')

			row = layout.row()
			row.prop(context.scene.an7_circle_walker_settings, 'radius_min')
			row.prop(context.scene.an7_circle_walker_settings, 'radius_max')
			layout.prop(context.scene.an7_circle_walker_settings, 'radius_decay')

			layout.prop(context.scene.an7_circle_walker_settings, 'max_elements')
			layout.prop(context.scene.an7_circle_walker_settings, 'max_failures')
			layout.prop(context.scene.an7_circle_walker_settings, 'max_attempts')

			box = layout.box()
			if bpy.context.view_layer.objects.active.type == "MESH" and bpy.context.object.mode == "OBJECT":
				layout.operator(AN7_Circle_Walker.bl_idname)
				if len(context.scene.an7_circle_walker_settings.feedback_time) > 0 and bpy.context.preferences.addons['AN7_circleWalker'].preferences.show_feedback:
					boxcol=box.column()
					boxcol.label(text="Points created: " + context.scene.an7_circle_walker_settings.feedback_elements)
					boxcol.label(text="Successive fails: " + context.scene.an7_circle_walker_settings.feedback_failures) # Alternative: consecutive?
					boxcol.label(text="Total attempts: " + context.scene.an7_circle_walker_settings.feedback_attempts)
					boxcol.label(text="Processing Time: " + context.scene.an7_circle_walker_settings.feedback_time)
			...
			print(str(exc) + " | Error in the AN7 Circle Walker panel")

classes = (AN7CircleWalkerPreferences, AN7_Circle_Walker, an7CircleWalkerSettings, AN7TOOLS_PT_circle_walker)
...
def register():
	...
	bpy.types.Scene.an7_circle_walker_settings = bpy.props.PointerProperty(type=an7CircleWalkerSettings)

def unregister():
	for cls in reversed(classes):
		bpy.utils.unregister_class(cls)
	del bpy.types.Scene.an7_circle_walker_settings
